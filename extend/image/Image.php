...
class Image
{
    ...
    const IMAGE_WATER_NORTH_F =   10 ; //常量，fdd  距离上面
    const IMAGE_WATER_NORTH_E =   11 ; //常量，fdd  距离下面
    const IMAGE_WATER_NORTH_G =   12 ; //常量，fdd  距离下面
    const IMAGE_WATER_NORTH_H =   13 ; //常量，fdd  距离下面
    const IMAGE_WATER_NORTH_I =   14 ; //常量，fdd  距离下面
    const IMAGE_WATER_NORTH_J =   15 ; //常量，fdd  距离下面
    const IMAGE_WATER_NORTH_K =   16 ; //常量，fdd  距离下面
    const IMAGE_WATER_NORTH_L =   17 ; //常量，fdd  距离下面
    const IMAGE_WATER_NORTH_M =   18 ; //常量，fdd  距离下面
    const IMAGE_WATER_NORTH_N =   19 ; //常量，fdd  距离下面
    /* 翻转相关常量定义 */
    const FLIP_X = 1; //X轴翻转
    const FLIP_Y = 2; //Y轴翻转
    ...
    public function water($source, $locate = self::WATER_SOUTHEAST, $alpha = 100)
    {
        ...
        switch ($locate) {
            ...
            case self::WATER_WEST:
                ...
                break;
            /* 上居中水印 特殊 */
            case self::IMAGE_WATER_NORTH_F:
                $x = ($this->info['width'] - $info[0])/2;
                $y = 91;
                break;
            /* 上居中水印 特殊 */
            case self::IMAGE_WATER_NORTH_E:
                $x = ($this->info['width'] - $info[0])/2;
                $y = 884;
                break;
            /* fdd 右下角 特殊 */
            case self::IMAGE_WATER_NORTH_G:
                $x += 550;
                $y = 880;
                break; 
            default:
                         ...
                         $locate = self::WATER_SOUTHEAST, $offset = 0, $angle = 0)
    {
        ...
        switch ($locate) {
            ...
            case self::IMAGE_WATER_NORTH_G:
                ...
                $y = 1109;
                break; 
            case self::IMAGE_WATER_NORTH_H:
                $x = 550;
                $y = 1191;
                break;   
            case self::IMAGE_WATER_NORTH_I:
                $x = 566;
                $y = 1235;
                break;  
            case self::IMAGE_WATER_NORTH_J:
                $x = 90;
                $y = 940;
                break;  
            case self::IMAGE_WATER_NORTH_K:
                $x = 90;
                $y = 1010;
                break; 
            case self::IMAGE_WATER_NORTH_L:
                $x = 90;
                $y = 1070;
                break; 
            case self::IMAGE_WATER_NORTH_M:
                $x = 90;
                $y = 1150;
                break;  
            case self::IMAGE_WATER_NORTH_N:
                $x += ($this->info['width']  - $w)/2;
                $y = 857;
                break;
            default:
